...
from utils.numpy_utils import bilinear_interp
from dataloader.data_utils import FrameInfo
...
class VidDataset(Dataset):
    ...
    def construct_data_list(self, reflist, prefix, feature_type):
        ...
        """
        rgb_path = reflist[0].replace("00000.jpg", "%s-00.npy" % prefix)
        ...
        ) + "/%s-%s-00.npy" % (prefix, feature_type)#, group_id)
        ...
        camlist_fg = (
            reflist[0]
            .replace("JPEGImages", "Cameras")
            .replace("00000.jpg", "fg-00-canonical.npy")
        )  # fg

        # TODO load cams directly
        # TODO do not need to return crop2raw from data loader
        crop2raw_path = mask_path.replace("-00.npy", "-crop2raw-00.npy")
        is_detected_path = mask_path.replace("-00.npy", "-is_detected-00.npy")
