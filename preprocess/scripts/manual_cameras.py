...
def read_mask_img(img_path, crop_size, use_full, obj_idx):
    mask_img_path = img_path.replace("JPEGImages", "Annotations").replace(".jpg", "-%02d.jpg" % obj_idx)
...
def get_annotation(track, index, obj_idx):
    annot = load_annotation(track.frame_paths[index], obj_idx)
...
def init_fig(track, mesh_path, obj_idx):
    ...
    annot = get_annotation(track, 0, obj_idx)
...
class FigureTracker:
    def __init__(self, mesh_path, seqnames, vidname, config, obj_idx):
        self.se3_dict = {}
        ...
        self.fig, self.v_adj = init_fig(self, mesh_path, obj_idx)
...
def load_annotation(frame_path, obj_idx):
    crop_size = 256
    use_full = False
    mask_img = read_mask_img(frame_path, crop_size, use_full, obj_idx)
    return mask_img
...
def caminfo_to_rotation(track):
    ...
    save_dir = (
        "database/processed_%s/Cameras/Full-Resolution/%s" % (track.vidname, track.seqnames[track.vid])
    )
    os.makedirs(save_dir, exist_ok=True)
    save_path = "%s/%02d-manual.json" % (save_dir, track.obj_idx)
...
def update_fig_annot(fig, track, index):
    new_annot = get_annotation(track, index, track.obj_idx)
...
def manual_camera_interface(vidname, obj_idx, mesh_path):  ### add obj_idx to some functions because read_mask_img
    ...
    track = FigureTracker(mesh_path, seqnames, vidname, config, obj_idx)
