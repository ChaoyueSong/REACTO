...
def extract_dinov2_seq(seqname, crop_size, vidname, use_full, obj_idx, pca_save):
    ...
    for it, impath in enumerate(imglist):
        ...
        rgb, _, mask, _ = read_frame_data(
            impath, crop_size, use_full, obj_idx, with_flow=False
        )
...
def extract_dinov2(seqname, crop_size, vidname, obj_idx=None, gpulist=[0]):
    ...
    for it, impath in enumerate(imglist_all_perm[:100]):
        ...
        # rgb: (s, s, 3), 0-1
        rgb, _, mask, _ = read_frame_data(
            impath, crop_size, False, obj_idx, with_flow=False
        )
        rgb = (rgb * 255).astype(np.uint8)  # to RGB
        mask = mask.astype(np.uint8)
    ...
    for vidid in range(len(config.sections()) - 1):
        seqname = config.get("data_%d" % vidid, "img_path").strip("/").split("/")[-1]
        args.append((seqname, crop_size, vidname, True, obj_idx, pca_save))
        args.append((seqname, crop_size, vidname, False, obj_idx, pca_save))
...
if __name__ == "__main__":
    ...
    extract_dinov2(seqname, crop_size, obj_idx, gpulist=gpulist)
